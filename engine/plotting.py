...
def create_subplot_delta_plot(ax, datasets, model_a_means, model_b_means, significant_winners, 
    ...
    for i, (delta, winner) in enumerate(zip(deltas, significant_winners)):
        ...
        if winner == 'A' and 'Onsite Finetuned' in label_a:
            onsite_wins = True
        elif winner == 'B' and 'Onsite Finetuned' in label_b:
            onsite_wins = True
        elif winner == 'A' and 'Onsite Finetuned' not in label_a:
            other_wins = True
        elif winner == 'B' and 'Onsite Finetuned' not in label_b:
            other_wins = True
    ...
    ax.set_ylabel(f"Δ({label_b} - {label_a})", fontsize=9)
    ax.set_title(title, fontsize=11, fontweight='bold')
...
def create_6_subplot_delta_plots(results, output_base_dir):
    ...
    comparisons = [
        {
            'name': 'Local Models vs Onsite Finetuned',
            'model_a': 'persite',
            'model_b': 'fl_finetuned',
            'label_a': 'Local Model',
            'label_b': 'Onsite Finetuned',
            'subplot_label': '(i)'
        },
        {
            'name': 'Central Model vs Onsite Finetuned',
            'model_a': 'central', 
            'model_b': 'fl_finetuned',
        ...
        {
            'name': 'Pipeline 1 vs Onsite Finetuned',
            'model_a': 'pipeline1',
            'model_b': 'fl_finetuned', 
            'label_a': 'Pipeline 1 (FL Round-Based)',
            'label_b': 'Onsite Finetuned',
            'subplot_label': '(iii)'
        },
        {
            'name': 'Pipeline 2 vs Onsite Finetuned',
            'model_a': 'pipeline2',
            'model_b': 'fl_finetuned',
            'label_a': 'Pipeline 2 (Weighted FedAvg)',
            'label_b': 'Onsite Finetuned',
            'subplot_label': '(iv)'
        },
        {
            'name': 'Pipeline 3 vs Onsite Finetuned', 
            'model_a': 'pipeline3',
            'model_b': 'fl_finetuned',
            'label_a': 'Pipeline 3 (Unweighted FedAvg)',
            'label_b': 'Onsite Finetuned',
            'subplot_label': '(v)'
        }
    ]
        ...
        for comp in comparisons:
            ...
            for eval_dataset in datasets:
                if eval_dataset in results[task]:
                    ...
                    if comp['model_a'] == 'persite':
                        model1 = f'{eval_dataset}_persite'
                    ...
                    elif comp['model_b'] == 'persite':
                        model2 = f'{eval_dataset}_persite'
                    else:
                        model2 = comp['model_b']
    ...
    for task in ['cup', 'disc']:
        ...
        for i, comp in enumerate(comparisons[:5]):
            ...
            if comp_name in comparison_data[task]:
                ...
                if comp['subplot_label']:
                    subplot_title = f"{comp['subplot_label']} Onsite Finetuned vs {comp['label_a']}"
                else:
                    subplot_title = f"Onsite Finetuned vs {comp['label_a']}"
        ...
        fig.text(0.5, 0.05, '(iii)', ha='center', va='bottom', fontsize=14, fontweight='bold')
        
        from matplotlib.patches import Patch
        legend_elements = [
            Patch(facecolor='lightblue', edgecolor='blue', hatch='///', 
                  label='Onsite Finetuned significantly better', alpha=0.8),
        ...
        for eval_dataset in datasets:
            ...
            if eval_dataset in results[task]:
                ...
                for train_dataset in datasets:
                    local_model = f'{train_dataset}_persite'
...
def convert_significant_winner_sk_fl(winner, fl_mean, sk_mean, p_value):
    """Convert significant winner to Onsite/Local/None format - legacy name kept for compatibility"""
    if winner == "None":
        return 'None'
    ...
    if 'fl_finetuned' in winner_str:
        return 'Onsite'
    elif 'sk_' in winner_str or '_persite' in winner_str:
        return 'Local'
    else:
        return 'None'

def convert_significant_winner_persite_fl(winner, fl_mean, persite_mean, p_value):
    ...
    if 'fl_finetuned' in winner_str:
        return 'Onsite'
    elif '_persite' in winner_str:
        return 'Local'
    else:
        return 'None'

def create_comprehensive_persite_vs_fl_plots(results, output_base_dir):
    """Create comprehensive SK vs FL fine-tuned comparison plots"""
    
    print("\n=== Creating comprehensive local model vs onsite finetuned plots ===")
    ...
    for task in ['cup', 'disc']:
        ...
        fig.suptitle(f'Local Model vs Onsite Finetuned Models - {task.upper()} Segmentation', 
                 fontsize=16, fontweight='bold', y=0.98)
        ...
        for eval_idx, eval_dataset in enumerate(datasets):
            ...
            if eval_dataset in results[task]:
                ...
                for persite_dataset in datasets:
                    fl_model = f'{persite_dataset}_fl_finetuned'
                    persite_model = f'{persite_dataset}_persite'
                    
                    comparison = get_comparison_stats(df, fl_model, persite_model)
                    
                    if comparison:
                        fl_means.append(comparison['model1_mean'])
                        persite_means.append(comparison['model2_mean'])
                        
                        delta = comparison['model1_mean'] - comparison['model2_mean']
                        deltas.append(delta)
                        
                        winner = convert_significant_winner_persite_fl(
                            comparison['significant_winner'],
                            comparison['model1_mean'],
                            comparison['model2_mean'],
                            comparison['corrected_p']
                ...
                ax.axhline(0, color="black", linewidth=0.8)
                ax.set_title(f'Evaluated on {eval_dataset.upper()}', fontsize=12, fontweight='bold')
                ax.set_xlabel('Dataset', fontsize=10)
                ax.set_ylabel('Δ(Onsite Finetuned - Local Model)', fontsize=10)
                ax.set_xticks(x_positions)
...
# ============================================================================
# PLOT 4: FL BASELINE COMPARISON PLOTS
# ============================================================================

def convert_significant_winner_fl_baseline(winner, model1_name, model2_name):
    """Convert significant winner to A/B format for FL baseline comparison"""
    if winner == "None":
        return 'None'
...
def create_subplot_fl_comparison(ax, datasets, model_a_means, model_b_means, 
    ...
    """Create a single subplot for FL baseline comparison"""
    ...
    ax.axhline(0, color="black", linewidth=0.8)
    ax.set_ylabel(f"Δ({label_a} - {label_b})", fontsize=9)
    ax.set_title(title, fontsize=11, fontweight='bold')
    ax.set_xticks(x_positions)
    ax.set_xticklabels(datasets, fontsize=8, rotation=45, ha='right')
    ax.tick_params(axis='y', labelsize=8)
...
def create_fl_baseline_comparison_plots(results, output_base_dir):
    """Create FL baseline comparison plots"""
    
    print("\n=== Creating FL baseline comparison plots ===")
    
    datasets = ['binrushed', 'chaksu', 'drishti', 'g1020', 'magrabi', 'messidor', 'origa', 'refuge', 'rimone']
    
    fl_models = [
        {'name': 'pipeline1', 'label': 'Pipeline 1 (FL Round-Based)'},
        {'name': 'pipeline2', 'label': 'Pipeline 2 (Weighted FedAvg)'},
        {'name': 'pipeline3', 'label': 'Pipeline 3 (Unweighted FedAvg)'}
    ]
    
    baselines = [
        {'name': 'persite', 'label': 'Local Model'},
        {'name': 'central', 'label': 'Central Model'}
    ]
    ...
    for task in ['cup', 'disc']:
            ...
            for fl_model in fl_models:
                ...
                for eval_dataset in datasets:
                    if eval_dataset in results[task]:
                        ...
                        if baseline['name'] == 'persite':
                            baseline_model = f'{eval_dataset}_persite'
                        else:
        ...
        fig, axes = plt.subplots(2, 3, figsize=(18, 12))
        fig.suptitle(f'FL Base Models vs Central and Local Models - {task.upper()} Segmentation', 
                     fontsize=16, fontweight='bold', y=0.95)
        ...
        legend_elements = [
            Patch(facecolor='lightblue', edgecolor='blue', hatch='///', 
                  label='FL model significantly better', alpha=0.8),
            Patch(facecolor='lightcoral', edgecolor='red', hatch='...', 
                  label='Central/Local models significantly better', alpha=0.8),
            Patch(facecolor='lightgray', edgecolor='gray', 
                  label='No significant difference', alpha=0.6)
        ]
...
def create_subplot_fl_vs_local(ax, training_datasets, fl_means, local_means, 
    ...
    ax.axhline(0, color="black", linewidth=0.8)
    ax.set_ylabel(f"Δ({fl_label} - Local)", fontsize=9, rotation=90)
    ax.set_title(f'Eval: {eval_dataset}', fontsize=11, fontweight='bold')
    ax.set_xticks(x_positions)
    ax.set_xticklabels([d.upper() for d in training_datasets], 
                       fontsize=8, rotation=45, ha='right')
...
def create_fl_vs_all_local_comparisons(results, output_base_dir):
    ...
    for fl_model in fl_models:
        for task in ['cup', 'disc']:
            ...
            fl_name_clean = fl_model['name'].replace('pipeline1', 'fl_round_based').replace('pipeline2', 'weighted_fedavg').replace('pipeline3', 'unweighted_fedavg')
    ...
    for task in ['cup', 'disc']:
        ...
        for eval_dataset in datasets:
            if eval_dataset in results[task]:
                ...
                local_model = f'{eval_dataset}_persite'
                central_model = 'central'
...
def extract_out_of_distribution_wins(disc_results_dir=None, cup_results_dir=None):
    ...
    datasets = ['binrushed', 'chaksu', 'drishti', 'g1020', 'magrabi', 'messidor', 'origa', 'refuge', 'rimone']
    persite_models = [f'{dataset}_persite' for dataset in datasets]
    
    # Model types to analyze
    model_types = {
        'Central Model': 'central',
        'Pipeline 1 (FL Round-Based)': 'pipeline1', 
        'Pipeline 2 (Weighted FedAvg)': 'pipeline2',
        'Pipeline 3 (Unweighted FedAvg)': 'pipeline3'
    }
    
    # Add FL finetuned models (these are site-specific)
    for dataset in datasets:
        model_types[f'Onsite Finetuned ({dataset.upper()})'] = f'{dataset}_fl_finetuned'
    ...
    for task in ['cup', 'disc']:
        ...
        # Initialize results matrix: rows=model_types, columns=persite models
        win_matrix = np.zeros((len(model_types), len(persite_models)), dtype=int)
            ...
            if csv_file.exists():
                try:
                    ...
                    for model_idx, (model_name, model_id) in enumerate(model_types.items()):
                        ...
                        # For each persite model
                        for persite_idx, persite_model in enumerate(persite_models):
                            persite_dataset = persite_model.replace('_persite', '')
                            
                            # Only count out-of-distribution comparisons
                            # (skip if evaluation dataset matches persite model dataset)
                            if eval_dataset != persite_dataset:
                                comparison = get_comparison_winner_heatmap(df, model_id, persite_model)
                                if comparison == model_id:
                                    win_matrix[model_idx, persite_idx] += 1
...
def create_ood_wins_heatmaps(output_base_dir, disc_results_dir=None, cup_results_dir=None):
    ...
    for task in ['cup', 'disc']:
        ...
        persite_models = results[task]['persite_models']
        
        # Remove '_persite' suffix from persite model names for display
        persite_model_labels = [persite_model.replace('_persite', '').upper() for persite_model in persite_models]
        
        # Create heatmap
        sns.heatmap(win_matrix,
                    xticklabels=persite_model_labels,
        ...
        explanation = ("Number of Wins: Number of times each model type significantly outperformed\n"
                      ...
                      "Each Onsite Finetuned model is compared against all local models for cross-site evaluation.\n"
...
def create_ood_summary_table(results, output_dir):
    ...
    # Add note
    note_text = ("Models ranked by total number of wins. Each Onsite Finetuned model\n"
...
def main():
    ...
    create_comprehensive_persite_vs_fl_plots(results, output_base_dir)
